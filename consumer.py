...
one set of settings. The loop is the audio consumer's (~/offline-mom-api/api/consumer.py), with its lessons kept:
...
def start():
    """Start the consumer thread. Raises on a misconfigured JOB_KIND, which fails startup.

    Deliberately fatal rather than logged and ignored: a pod that came up "healthy" while reading
    nobody's topic is the failure that takes a day to notice. FastAPI's startup hook lets the
    exception through, so the pod crash-loops with the reason in its log.
    """
    global _thread
    if JOB_KIND != "prompt":
        raise SystemExit(f"JOB_KIND must be 'prompt' for this service, not {JOB_KIND!r}. "
                         "Minutes from a recording are JOB_KIND=mom in offline-mom-api.")
